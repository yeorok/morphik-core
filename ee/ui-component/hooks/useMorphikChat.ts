...
export function useMorphikChat({
  ...
  const append = useCallback(async (message: Omit<UIMessage, 'id' | 'role' | 'createdAt'>) => {
    ...
    try {
      ...
      if (currentQueryOptions.filters) {
        try {
          parsedFilters = typeof currentQueryOptions.filters === 'string'
      ...
      if (!isLoading) { /* Only set if it wasn't already set by error block */ }
      setIsLoading(false);
    }
